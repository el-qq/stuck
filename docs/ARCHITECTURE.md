...
- `frontend/components/` is grouped by UI boundary: `auth/`, `trace/`,
  `rules/`, `shell/` and top-level `screens/`.
...
- `LoginScreen` is a page frame; `useLoginForm` owns non-secret form state,
  bootstrap/prefill effects and login submission while the card and credentials
  components render the accessible controls.
